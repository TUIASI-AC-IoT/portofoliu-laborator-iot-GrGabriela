...
#define GPIO_INPUT_I 2
#define GPIO_INPUT_BUTTON (1ULL<<GPIO_INPUT_I)
...
static void gpio_task_example(void* arg)
{
    ...
    for (;;) {
        if (xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
            if(!gpio_get_level(GPIO_INPUT_I)){
            //tx_buffer = "GPIO4=1";
            printf("GPIO[%"PRIu32"] intr, val: %d\n", io_num, gpio_get_level(io_num));
            cnt++;
            printf("cnt: %d\n", cnt);
            //int len = sendto(sock, tx_buffer, sizeof(tx_buffer) - 1, 0, &source_addr_python, &sock_python_len );
...
static void button_send(){
    ...
    io_conf.pull_down_en = 0;
     ...
     //bit mask of the pins, use GPI02 here
     io_conf.pin_bit_mask = GPIO_INPUT_BUTTON;
     //set as input mode
    ...
    gpio_set_intr_type(GPIO_INPUT_I, GPIO_INTR_ANYEDGE);
    ...
    gpio_isr_handler_add(GPIO_INPUT_I, gpio_isr_handler, (void*) GPIO_INPUT_I);

    // //remove isr handler for gpio number.
    // gpio_isr_handler_remove(GPIO_INPUT_I);
    ...
    while(1)
    {
        ...
        while (1) {
            ...
            // Data received
                ...
                int aux = strcmp(rx_buffer, "GPIO4=1");
                if(aux == 0){
                    //vTaskDelay(1000 / portTICK_PERIOD_MS);
